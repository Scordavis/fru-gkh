...
<link rel="stylesheet" href="/admin_rc/css/bootstrap.min.css" />
...
?>
<!-- Загальна кількість будинків:<?=$posts?> --> <!-- Scor edit -->
<?
...
?>
		<!-- .site-header --><!-- Scor edit -->
	<header class="container-fluid site-header">
		<div class="container h-100">
			<div class="row h-100 align-items-center">
				<div class="col-4 text-left site-header__title-block">
					<h1 class="mb-0 site-header__title">Таблица<span class="site-header__region">Подтаблица</span></h1>
					<p class="mt-0 text-capitalize site-header__city"><span class="text-lowercase mr-1">м.</span>Город</p>
					<p class="mt-0 text-lowercase site-header__city">Загальна кількість будинків: <?=$posts?></p>
				</div>
				<div class="col-2 text-left site-header__logo-wrapper">
					<img src="http://manage.info-gkh.com.ua/logo.png" alt=""><!--  class="site-header__logo" -->
				</div>
				<button class="col mx-2 py-3 button button--white site-header__button">Кнопка 1</button>
				<button class="col mx-2 py-3 button button--white site-header__button">Без фильтра</button>
				<button data-fancybox data-src="#tableModal" class="col mx-2 py-3 button button--white site-header__button"><i class="fa fa-sliders" style="color: red; width: 1px; position: relative; right: 22px;"></i>Фільтри</button>
			</div>
		</div>
	</header>
...
<?php
  if(!isset($_GET['search'])) {
    ...
    {?>
    	...
    	<!-- pagination -->
    	<!--<span class='paginate'>
    		<a style='font-weight: bold; font-size: 20px;' href='/ReestrBudinkiv/dataeditor_1step/?id=<?=$factory_id?>&mini=<?=$mini?>&misto=<?=$misto?>&region=<?=$region?>&start=1'>&laquo;</a>
    	</span>
    	<span class='paginate'>
    		<a style='font-weight: bold; font-size: 20px;'
    		href='/ReestrBudinkiv/dataeditor_1step/?id=<?$factory_id?>&mini=<?$mini?>&misto=<?$misto?>&region=<?$region?>&start=<?=($page-1)?>'>< </a>
    	</span>-->
    ...
    for($i=$start_loop; $i<=$end_loop; $i++)
    {
		if($i>0) {
			...
			<?}else {?>
				<!--
				<span class='paginate'>
					<a href='/ReestrBudinkiv/dataeditor_1step/?id=<?=$factory_id?>&mini=<?=$mini?>&misto=<?=$misto?>&region=<?=$region?>&start=<?=$i?>'> <?=$i?> </a>
				</span>-->
			<?}
    ...
    {?>
    	<!--<span class='paginate'>
    		<a style='font-weight: bold; font-size: 20px;' href='/ReestrBudinkiv/dataeditor_1step/?id=<?=$factory_id?>&mini=<?=$mini?>&misto=<?=$misto?>&region=<?=$region?>&start=<?=($page + 1)?>'> ></a>
    	</span>

   <span class='paginate'>
   	<a style='font-weight: bold; font-size: 20px;' href='/ReestrBudinkiv/dataeditor_1step/?id=<?=$factory_id?>&mini=<?=$mini?>&misto=<?=$misto?>&region=<?=$region?>&start=<?=$total_pages?>'>&raquo;</a>
   </span>-->
    <?}
	...
	<nav aria-label="...">
		<ul class="justify-content-center pagination">
			<li class="page-item disabled">
				<span class="px-3 page-link">&laquo;</span>
			</li>
			<li class="page-item disabled">
				<span class="px-3 page-link">&#60;</span>
			</li>
			<li class="page-item">
				<a class="px-3 page-link" href="#">1</a>
			</li>
			<li class="page-item active">
				<span class="px-3 page-link">
					2
					<span class="sr-only">(текущий)</span>
				</span>
			</li>
			<li class="page-item">
				<a class="px-3 page-link" href="#">3</a>
			</li>
			<li class="page-item disabled">
				<a class="px-3 page-link" href="#">...</a>
			</li>
			<li class="page-item">
				<a class="px-3 page-link" href="#">56</a>
			</li>
			...
			<li class="page-item">
				<a class="px-3 page-link" href="#">&raquo;</a>
			</li>
			<li class="ml-2 page-item pagination-input" >
				<form action="">
					<input class="px-3 w-25 form-control" type="text" placeholder="№">
				</form>
			</li>
		</ul>
	</nav>
...
.paginate a{
	...
	border: none;
	/*border:1px solid #DCDCDC;*/ /*Scor edit*/
...
.xsearch_result li:hover{
 ...
 .hh {
		height:20px;overfow:hidden;
						...
						<div class="w-100 tblWrapper" style="min-width:800px;">
...
<div class="container">
	<!-- form -->
	<div id="tableModal" style="background-color: white; display: none;">
		<!-- wrapper for padding -->
		<div class="pt-4">
			<form action="" class="row z-index-3 mb-2 py-3  emergency-service__form-wrapper">
				<!-- street -->
				<div class="col-6 mb-3 emergency-service__form-input">
					<div class="input-group">
						<input type="text" class="p-3 py-4 w-100" id="streetInput" placeholder="Оберіть вулицю">
					</div>
					<ul class="pl-0 emergency-service__form-list" style="display: none;">
						<li class="px-3 py-2">Каземирская</li>
						<li class="px-3 py-2">Причальная</li>
						<li class="px-3 py-2">Ростовская</li>
						<li class="px-3 py-2">Алая</li>
					</ul>
				</div>
				<!-- house -->
				<div class="col-6 mb-3 emergency-service__form-input">
					<div class="input-group">
						<select class="p-3 custom-select" id="select-street">
							<option selected>Номер будинку</option>
							<option value="1">1</option>
							<option value="2">1а</option>
							<option value="3">1б</option>
							<option value="4">2</option>
							<option value="5">3</option>
							<option value="6">3а</option>
						</select>
					</div>
				</div>
				<!-- stage -->
				...
				<div class="col-6 mb-3 emergency-service__form-input">
					<div class="input-group">
						<select class="p-3 custom-select" id="select-street">
							<option selected>Послуги</option>
							...
							<option value="7">послуга 7</option>
							<option value="8">послуга 8</option>
							<option value="9">послуга 9</option>
						</select>
					</div>
				</div>
				<!-- apartment -->
<!-- 				<div class="col-lg-4 col-md-6 col-sm-6 col-12 mb-3 emergency-service__form-input">
					<div class="input-group">
						<select class="p-3 custom-select" id="select-street">
							<option selected>Номер квартири</option>
							<option value="1">1</option>
							<option value="2">2</option>
							<option value="3">3</option>
							<option value="4">4</option>
				...
				<!-- damage -->
<!-- 				<div class="col-lg-8 col-md-8 col-sm-7 col-12 mb-3 emergency-service__form-input">
					<div class="input-group">
						<select class="p-3 custom-select" id="select-street">
							<option selected>Оберіть вид пошкодження</option>
							<option value="1">Поламана свістюлька</option>
							<option value="2">Поламана кішка</option>
							<option value="3">Не працює зять</option>
							<option value="4">Долар по 27 гривень</option>
							<option value="5">По телику одна маячня</option>
							<option value="6">Чухається щось...</option>
						</select>
					</div>
				</div> -->
				<!-- submit -->
				<div class="col-4 emergency-service__form-input">
					<button type="submit" class="w-100 py-4 emergency-service__form-btn emergency-service__form-btn--red">Готово<i class="pl-2 fa fa-angle-double-right"></i></button>
				</div>
			</form>
		</div>
		<!-- end wrapper for padding -->
	</div>
</div>
...
.divFixHeaderCol {
  position:absolute;
  width: 100% !important; /*Scor edit*/
  overflow-y: auto; /*Scor edit*/
  border:0;
