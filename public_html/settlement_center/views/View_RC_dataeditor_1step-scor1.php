	...
	<header class="container-fluid site-header">
		<div class="container h-100">
			<div class="row h-100 align-items-center">
				...
				<button class="col mx-2 py-3 py-lg-2 button button--white site-header__button">Житловий фонд</button>
				<button class="col mx-2 py-3 py-lg-2 button button--white site-header__button">Бюджет</button>
				<button class="col mx-2 py-3 py-lg-2 button button--white site-header__button">Інші споживачі</button>
			</div>
		</div>
	</header>
...
<?php
  if(!isset($_GET['search'])) {
    ...
    if($page > 1)
    {?>
     <span class='paginate'><a style='font-weight: bold; font-size: 20px;' href='/ReestrBudinkiv/dataeditor_1step/?id=<?=$factory_id?>&mini=<?=$mini?>&misto=<?=$misto?>&region=<?=$region?>&start=1'>&laquo;</a></span>
    <span class='paginate'>
	<a style='font-weight: bold; font-size: 20px;' 
	href='/ReestrBudinkiv/dataeditor_1step/?id=<?$factory_id?>&mini=<?$mini?>&misto=<?$misto?>&region=<?$region?>&start=<?=($page-1)?>'>< </a></span><?
    }
    for($i=$start_loop; $i<=$end_loop; $i++)
    {
		if($i>0) {
			...
			<?}else {?>
				...
				<span class='paginate'>
				<a href='/ReestrBudinkiv/dataeditor_1step/?id=<?=$factory_id?>&mini=<?=$mini?>&misto=<?=$misto?>&region=<?=$region?>&start=<?=$i?>'> <?=$i?> </a></span>
			<?}
   ...
   <span class='paginate'><a style='font-weight: bold; font-size: 20px;' href='/ReestrBudinkiv/dataeditor_1step/?id=<?=$factory_id?>&mini=<?=$mini?>&misto=<?=$misto?>&region=<?=$region?>&start=<?=$total_pages?>'>&raquo;</a></span>
    <?}
